...
for dir in /mnt/filesystem/*; do
  ...
  if ! tar -vczf "$nightly_tar" -C "$dir" . --warning=no-file-changed; then
    ok=
    continue
  fi

  echo "[INFO] Uploading tarball of $env to S3"
  if ! aws --debug s3 mv "$nightly_tar" "s3://$BACKUPS_BUCKET/files/$BACKUPS_SITE/$timestamp/$env.tar.gz"; then
    ok=
    continue
  fi
