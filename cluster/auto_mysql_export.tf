        ...
        inputs = {
          ...
          Parameters = {
            commands = [
              ...
              "echo -n 'host=' && jq -r '.SecretString.host' cred.json >>/etc/my.cnf",
              "echo -n 'port=' && jq -r '.SecretString.port' cred.json >>/etc/my.cnf",
              "echo -n 'user=' && jq -r '.SecretString.user' cred.json >>/etc/my.cnf",
              "echo -n 'password=' && jq -r '.SecretString.password' cred.json >>/etc/my.cnf",

              # Run `mysqldump` against the DB
              "docker run --rm -it -v /etc/my.cnf:/etc/my.cnf:ro mysql:8.0 mysqldump --set-gtid-purged=OFF --no-tablespaces {{ site }}-{{ environment }}-{{ database }} >dump.sql",
